...
test("HUD compass and action clusters retain continuous and grid-aligned geometry", async () => {
  ...
  assert.match(styles, /prototype-hud__action-vital[\s\S]*width: var\(--prototype-action-cell[\s\S]*height: calc\(var\(--prototype-action-cell[\s\S]*\* 6\)/);
  assert.match(styles, /prototype-hud__action-vital[\s\S]*left: 50%;[\s\S]*transform: translateX\(-50%\)/);
  assert.match(styles, /grid-template-rows: repeat\(6, var\(--prototype-action-cell\)\)/);
