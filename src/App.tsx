...
import { ACCOUNT_SOUL_ITEM, CHARACTER_SOUL_ITEM, GARDEN_WORLD_ITEM } from "./inventory/inventory-model";
...
function SystemCharacterSelect({ projection, worlds, busy, message, onSelect, onCreate, onWake }: Readonly<{
  ...
  return <main className="system-stage system-theme system-character-stage" onDragOver={(event) => { if (!heldCharacterRef.current) return; event.preventDefault(); updateHeldPointer({ x: event.clientX, y: event.clientY }); }} onDrop={(event) => { if (!heldCharacterRef.current) return; event.preventDefault(); seatHeldCharacter({ x: event.clientX, y: event.clientY }); }} onPointerMove={(event) => { if (heldCharacterRef.current) updateHeldPointer({ x: event.clientX, y: event.clientY }); }} onPointerUp={(event) => { if (heldCharacterRef.current) seatHeldCharacter({ x: event.clientX, y: event.clientY }); }}>
    <aside className="system-spirit-slot" tabIndex={0} aria-label={`${accountSpirit.item.name}. Account statistics`}>
...
export function App() {
  ...
  if (flow.phase === "character-select" || flow.phase === "character-ready") return <><SystemCharacterSelect projection={flow.projection} worlds={worldDiscovery} busy={busy} message={flow.message} onSelect={(characterId) => void selectCharacter(flow.projection, characterId)} onCreate={() => navigate("/characters/new")} onWake={() => void enterGarden({ phase: "gateway-entry", projection: flow.projection, worldId: worldDiscovery?.defaultWorldId ?? "garden" })} /><SeatedAwareness disabled={poweringDown} onRemove={logout} /></>;
