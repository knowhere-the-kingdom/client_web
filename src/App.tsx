...
function KeyGate({ busy, message, onUnlock }: Readonly<{ busy: boolean; message: string | null; onUnlock: () => void }>) {
  ...
  return <main className="inventory-login-designer atlas-power-gate" aria-label="Designer access">
    ...
    <div className="inventory-login-designer__item"><InventoryItemCard definition={AWARENESS_ITEM} instance={AWARENESS_INSTANCE} held={movement.heldInstanceId === AWARENESS_INSTANCE.instanceId} disabled={busy} onPickUp={() => setMovement(pickUpInventoryItem(AWARENESS_INSTANCE))} onCancel={() => setMovement(cancelInventoryMovement())} /></div>
    <div className="inventory-login-designer__slot"><InventorySlot definition={DESIGNER_RECEPTACLE} heldItem={movement.heldInstanceId ? { definition: AWARENESS_ITEM, instance: AWARENESS_INSTANCE } : null} className="designer-slot" disabled={busy} onPlace={() => { const placed = placeHeldInventoryItem(movement, AWARENESS_INSTANCE, true); if (placed.ok) { setMovement(placed.state); onUnlock(); } }} onCancel={() => setMovement(cancelInventoryMovement())}><span className="atlas-designer-keyhole" aria-hidden="true" /></InventorySlot></div>
    <p className="inventory-login-designer__hint" role={message ? "alert" : undefined}>{message ?? (busy ? "Preparing your Garden…" : "Move Awareness into the Designer Slot")}</p>
  </main>;
}
