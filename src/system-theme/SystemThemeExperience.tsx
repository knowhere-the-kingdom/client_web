...
type Props = Readonly<{ gateway: GatewayClient }>;

export function SystemThemeExperience({ gateway }: Props) {
  ...
  async function checkSession() {
    ...
    if (result.ok) dispatch({ type: "session-ready", generation: g, projection: result.value });
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    progress(g, correlationId, 2, 100, "Message received"); dispatch({ type: "session-ready", generation: g, projection: result.value });
  }
  if (flow.stage === "splash" || flow.stage === "identified" || flow.stage === "designer-ready") return <main className="system-stage system-theme system-splash" onDragOver={(e) => e.preventDefault()} onDrop={(e: DragEvent) => { e.preventDefault(); insertKey(); }}>
  ...
  const panel = flow.stage === "registering" ? <section><h1>Register</h1><p>Registration is unavailable until the required phone-based contract is approved.</p><button type="button" onClick={() => dispatch({ type: "show-login" })}>Back to login</button></section>
    ...
    : flow.stage === "login" ? <form onSubmit={login}><h1>KNOWHERE</h1><label>Username<input name="username" required autoComplete="username" /></label><label>Password<input name="password" type="password" required autoComplete="current-password" /></label><label className="system-check"><input name="remember" type="checkbox" disabled /> Remember me (unavailable)</label><button type="submit">Login</button><button type="button" onClick={() => dispatch({ type: "show-recovery" })}>Forgot Password</button><div className="system-divider">or</div><button type="button" disabled title="Discord state and PKCE handshake is not configured">Login with Discord</button><button type="button" onClick={() => dispatch({ type: "show-register" })}>Register</button><div className="system-qr" role="img" aria-label="Scan to Login placeholder. No code is encoded."><span aria-hidden="true">QR</span><small>Scan to Login</small></div></form>
